...
if (!empty($_POST) ) {
   ...
   if ( !isset($_POST['mdp']) || strlen($_POST['mdp']) < 2 || strlen($_POST['mdp']) > 50) {
    ...
    if ( !isset($_POST['email']) || strlen($_POST['email']) > 50 || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
        ...
        $contenu .= '<div class="alert alert-danger">Votre email pas conforme</div>';
        }
        ...
        <?php echo $contenu ?>
            <form action="" method="POST" class="border border-info">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="mdp" id="mdp" class="form-control" placeholder="Votre mot de passe"></input>
                </div>
